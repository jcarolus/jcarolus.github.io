---
layout: default
---

## Chess

- **Multiple game variants** — Standard chess, Chess 960, and Duck chess (experimental)
- **Play online** — Connect to [Lichess](https://lichess.org) and [FICS](https://freechess.org)
- **Puzzles** — Lichess puzzle collection and mate-in-two challenges
- **Chromecast support** — Cast the board to your TV
- **Opening explorer** — Browse the ECO opening encyclopedia
- **Game database** — Store and search your games locally
- **Accessibility** — Screen reader support via TalkBack
- **Board editor** — Set up custom positions manually
- **Engine support** — Plug in third-party chess engines via the Open Exchange Protocol (OEX)

---

## Demo

[Watch on YouTube](https://youtu.be/oFvqyaLGvq0)

## Install

| Store | Link |
|---|---|
| F-Droid | [f-droid.org/packages/jwtc.android.chess](https://f-droid.org/packages/jwtc.android.chess/) |
| Google Play | [play.google.com/store/apps/details?id=jwtc.android.chess](https://play.google.com/store/apps/details?id=jwtc.android.chess) |

## Source code

[github.com/jcarolus/android-chess](https://github.com/jcarolus/android-chess)

## Help

Questions or issues? Join the discussion group:
[groups.google.com/g/jwtc](https://groups.google.com/g/jwtc)

## Support the project

If you enjoy the app, consider buying me a coffee:
[paypal.me/jcaroluschess](https://paypal.me/jcaroluschess)
